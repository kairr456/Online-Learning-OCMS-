...
<div class="account-manager-container">
    <div class="dashboard-title">Manage Course Categories</div>

    <!-- Thông báo kết quả thao tác -->
    <c:if test="${param.msg == 'deleted'}">
        <div class="alert-box alert-success">
            <i class="fa-solid fa-circle-check"></i> Course category deleted successfully!
        </div>
    </c:if>
    <c:if test="${param.error == 'has_courses'}">
        <div class="alert-box alert-danger">
            <i class="fa-solid fa-triangle-exclamation"></i> Cannot delete this category because it contains existing courses!
        </div>
    </c:if>
    <c:if test="${param.error == 'delete_failed'}">
        <div class="alert-box alert-danger">
            <i class="fa-solid fa-triangle-exclamation"></i> Failed to delete course category. Please try again.
        </div>
    </c:if>

    <!-- ===== Toolbar (Search + Nút Thêm mới) ===== -->
    <form id="filterForm"
          action="${pageContext.request.contextPath}/admin/course-categories"
          method="GET"
    ...
    <!-- ===== Bảng danh sách danh mục Khóa học ===== -->
        ...
        <table class="account-table">
            <thead>
                <tr>
                    ...
                    <th>Description</th>
                    <th class="col-cat-blogs">Courses</th>
                ...
                <c:forEach var="cat" items="${categoryList}">
                    <tr>
                        ...
                        <td class="text-center">
                            <span class="badge active cat-badge-count">${cat.courseCount} courses</span>
                        </td>
                        <td>${cat.createdAt}</td>
                        <td>${cat.updatedAt}</td>
                        <td class="action-cell">
                            ...
                            <!-- Nút Xóa (Chỉ cho phép xóa khi courses = 0) -->
                            <c:choose>
                                <c:when test="${cat.courseCount == 0}">
                                    <a href="${pageContext.request.contextPath}/admin/course-categories?action=delete&id=${cat.id}"
                                       class="btn-action delete"
                                       onclick="return confirm('Are you sure you want to delete course category \'${fn:escapeXml(cat.name)}\'?')"
                                       title="Delete">
                                ...
                                <c:otherwise>
                                    <button type="button"
                                            ...
                                            disabled
                                            title="Cannot delete: Category contains ${cat.courseCount} course(s)">
                                        <i class="fa-regular fa-trash-can"></i>
                                    </button>
                                </c:otherwise>
                            </c:choose>
...
<!-- ===== Modal Add / Edit Course Category ===== -->
<div id="categoryModal" class="modal" style="display:none;">
    <div class="modal-content">
        <span class="modal-close" onclick="closeModal()">&times;</span>
        <h3 id="modalTitle" class="cat-modal-title">Add Course Category</h3>
        ...
        <form id="categoryForm">
            ...
            <input type="text" id="f_name" name="name" placeholder="e.g. Programming, Business, Design..." maxlength="100" required>
            
            <label for="f_description">Description</label>
            <textarea id="f_description" name="description" rows="4" placeholder="Brief summary about this course category..." maxlength="500" class="cat-textarea"></textarea>
